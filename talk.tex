...
\begin{frame}{Electroweak Symmetry Breaking}
  ...
  \begin{columns}
    ...
    Higgs potential:
    \begin{align*}
      V(\phi) = \mu^2 \phi^\dag \phi + \lambda (\phi^\dag \phi)^2 \qquad (\mu^2 < 0)
    \end{align*}
    ...
    \begin{tikzpicture}
      ...
      \filldraw[color=Red,fill opacity=0.85,draw opacity=0] (0,0.7) circle (3.5pt);

      \filldraw[color=Red,fill opacity=0.85,draw opacity=0] (-1.55,-0.6) circle (3.5pt);

      \draw[stealth-, line width=1pt, color=Red] (-1.55,-0.6) -- (0,0.7);
      %node[above,black,]{\footnotesize \ttbar + true-\tauhadvis};

    \end{tikzpicture}
  \end{columns}

  Higgs potential after EWSB in unitary gauge ($\phi = \frac{1}{\sqrt{2}} (0, v + H)^{\text{T}}$):
  \begin{align*}
