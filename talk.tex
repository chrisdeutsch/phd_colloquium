...
\begin{frame}{Event Selection}
  \begin{center}
    \begin{tabular}{c@{\hskip 2em}c@{\hskip 3em}c}
      ...
      \textcolor{hhblue}{no $e$ or $\mu$} & \multicolumn{2}{c}{\textcolor{lhred}{exactly one $e$ or $\mu$}} \\
...
\begin{frame}{Background Estimation}
  \begin{tikzpicture}[remember picture,overlay,shift=(current page.south
    ...
    % ------------------------------------------------------------------
    \onslide<2->{%
      \node[align=left] at (0.865, 0.65) {\allbold{Bkgs.\ with real \tauhadvis:}};

      \draw[stealth-, line width=2pt, color=toporange] (0.53, 0.38) -- (0.75, 0.5)%
      node[right, align=center]{\allbold{\ttbar with true \tauhadvis}\\[-0.25ex](\SI{39}{\percent})};

      \draw[stealth-, line width=2pt, color=zjetsgreen] (0.53, 0.32) -- (0.805, 0.32)%
    ...
    % ------------------------------------------------------------------
    \onslide<3->{%
      \node[align=left] at (0.14, 0.55) {\allbold{Bkgs.\ with mis-id.\ \tauhadvis:}};

      \draw[stealth-, line width=2pt, color=topfakered] (0.35, 0.41) -- (0.25,
      0.41)%
      node[left, align=center]{\allbold{\ttbar with fake \tauhadvis}\\[-0.25ex](\SI{25}{\percent})};

      \draw[stealth-, line width=2pt, color=fakeblue] (0.335, 0.31) -- (0.195,
      0.24)%
...
\begin{frame}{Motivate Signal Extraction}
\end{frame}
...
\begin{frame}{Post-Fit Discriminants}
  \vspace*{0.5em}
  \begin{columns}
  ...
  \begin{center}
    \only<1>{%
      \begin{overpic}[width=0.5\textwidth]{discussion/results_139ifb_onlybbtautau}
        \put(69,85){\footnotesize arXiv:2211.01216}
      \end{overpic}
    }%
    \only<2->{%
...
\begin{frame}{Summary \& Outlook}
  ...
  \item First evidence of SM HH production ($Z_0 > 3\sigma$) expected at HL-LHC
