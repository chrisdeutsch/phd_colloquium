...
\note{
  ...
  \item Lagrangian describes the dynamics of the fields

  \item Built on symmetry principles (local gauge invariance)

  \item Build up slide (first only radiation \& fermions) [Higgs sector after next bullet]

  \item Problem: if we want local gauge invariance to hold -> particles have to
    be massless. (Illustrate with image?)

  \item Instead dynamic approach is introduced

  \item Embed a doublet of complex scalar fields (Higgs field) in the SM
    symmetry
...
\begin{frame}{Electroweak Symmetry Breaking: 2D Analogue}
  \begin{center}
    \begin{tikzpicture}
      \node (potential) at (0,0) {%
      ...
      \onslide<2->{
        ...
        node[right, align=center]{%
          \footnotesize symmetric state\\[-0.5ex]
          \footnotesize (unstable)%
        };
      }
      ...
      \onslide<3->{
        ...
        \draw[stealth-stealth, line width=1pt, color=Blue] %
        (-1.95,-0.8) -- node[midway,below]{\footnotesize \allbold{$v$}} (0,-0.8);
      }
    \end{tikzpicture}
  \end{center}

  Write in terms of small radial perturbations: $V(H) = \lambda v^2 H^2 + \lambda v H^3 + \frac{\lambda}{4} H^4$
\end{frame}
...
\note{Put here a plot of Higgs boson mass measurements \& couplings}
...
\begin{frame}{Background Estimation: Multi-Jet}
  ...
  % TODO: A plot of the Multi-Jet VR would be nice here!
