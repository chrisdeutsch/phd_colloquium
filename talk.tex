...
\begin{frame}{Background Estimation: Overview}
  ...
  \resizebox{0.8\textwidth}{!}{%
    \begin{tabular}{lcc}
      ...
      Jet \ra fake-\tauhadvis (\ttbar) & \allbold{\textcolor{topfakered}{Simulation + data-driven scale factors}} & \multirow{2}{*}{\textcolor{fakeblue}{Combined fake-factor method}} \\[0.1em]
...
\begin{frame}{Background Estimation: \allbold{\ttbar} with Fake
  ...
  \begin{columns}[onlytextwidth]
    ...
    \allbold{Measured in $\ell + \tauhadvis$~CR} in bins of:\ \tauhadvis~\pT,
...
\begin{frame}{Background Estimation: Multi-Jet}
  \begin{columns}[onlytextwidth]
    ...
    \textbf{Fake Factor Method:}

    Opposite-sign (OS) / same-sign charge (SS)

    ID region: 2 loose \tauhadvis

    Anti-ID region: 1 loose \tauhadvis, 1 not-loose \tauhadvis but $RNN > 0.01$
    ...
    Details:\\
    - Binning in \tauhadvis \pT, \tauhadvis \Ntracks, \tauhadvis $\eta$, year\\
    - Actually do two different fake estimations and average\\
    - Do it in 1 btag region extrapolate to 2 btag region

    \column{0.4\textwidth}
    \centering

    \includegraphics[width=0.8\textwidth]{ff_sketch}
  \end{columns}
