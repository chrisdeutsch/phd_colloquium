...
\begin{frame}{Higgs Boson Pair Production in the SM}
  ...
  \begin{minipage}{\dimexpr 0.75\textwidth - 2\fboxsep}
    ...
    \hfill%
    \raisebox{0.35em}{\includegraphics[scale=0.85]{feynman_graphs/di_higgs_triangle}}%
    \hspace*{0.05\textwidth}
...
\begin{frame}{Final States of Searches for \allbold{\HH} Production}
  \begin{columns}[onlytextwidth]
    ...
    Trade-off between branching ratio \& ``cleanliness'' of final state
...
\begin{frame}{The \allbold{\bbtautau} Final State}
  ...
  \begin{columns}[onlytextwidth]
    ...
    \only<1>{\vspace*{-1pt}\includegraphics[width=0.97\textwidth]{final_state/final_state_lephad}}%
    \only<2>{\includegraphics[width=0.97\textwidth]{final_state/final_state_hadhad}}
  \end{columns}
\end{frame}
...
\begin{frame}{Signal Extraction}
  ...
  page.south east), y=(current page.north west), ]{%
    ...
    \node at (0.6575, 0.13 - 0.03) {\footnotesize \color{Blue} \textbf{background-like}};
    \draw[-stealth, line width=1.5pt, color=Red] (0.755,0.13) -- (0.91,0.13);
    \node at (0.8325, 0.13 - 0.03) {\footnotesize \color{Red} \textbf{signal-like}};
  }
\end{frame}
...
\begin{frame}[standout]
  Search for Resonant \allbold{\HH} Production
...
\begin{frame}{Search for Resonant \allbold{\HH} Production}
  ...
  \begin{columns}[onlytextwidth,c]
    ...
    \begin{overpic}[scale=1.0]{feynman_graphs/di_higgs_resonant}
    \end{overpic}
    ...
    \begin{itemize}
      \setlength{\itemsep}{0.3em}
    \item E.g.\ in extended Higgs sector models
  ...
  \allbold{Main difference compared to SM \HH search:}
  \begin{itemize}
  \item Signal extraction based on \textit{parameterised neural networks} (PNN)
  \end{itemize}
\end{frame}
...
\begin{frame}{Upper Limits on \allbold{$\sigma(pp \to X \to HH)$}}
  \begin{center}
  ...
  \item Most sensitive channel in
    $\SI{375}{\GeV} \leq m_{X} \leq \SI{800}{\GeV}$

  \item Excess at $\mX = \SI{1000}{\GeV}$ with global significance of
    $2.0\sigma$
...
\begin{frame}{Discovery Tests}
  ...
  %   $\hat{\sigma}(pp \to X \to HH) = \left( 17.4^{+7.5}_{-6.5}
  %   \right)\si{\femto\barn}$
  \item Local significance $= 3.1\sigma$
  \item Global significance $= 2.0\sigma$ (accounting for ``multiple testing'')
  \end{itemize}
